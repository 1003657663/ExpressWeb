        ...
        return (
            <div>
                <select key="selectsend" style={selectStyle} className="form-control" onChange={this.sendSiteChange}>
                    <option value="-1">请选择发出站点</option>
                    {this.props.data.map(function (d, index) {
                        return <option key={"option"+index} value={d.id}>{d.name}</option>
                    })}
                </select>
                <select key="selectreceive" style={selectStyle} className="form-control" onChange={this.receiveSiteChange}>
...
function placeChoiceSite() {
    ...
    Tools.myAjax({
        ...
        success: function (data) {
            showDialog("dialog","请选择包裹收发站点",<SelectSiteDialog getResult={getResult} data={data}/>,true,sureButtonClick);
            //弹出选择框
        }.bind(this),
        error: function (data) {
            console.info(data);
